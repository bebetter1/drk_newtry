...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if image.mode != 'RGBA' and os.path.exists(image_path.replace('images', 'mask')):
            mask_path = image_path.replace('images', 'mask')
            mask = Image.open(mask_path)
            # bilinearly resize mask to image size and leave the red channel as the mask, set nonzero values to 255
            mask = mask.resize((image.size[0], image.size[1]), Image.BILINEAR)
            mask = mask.convert('L')
            mask = np.array(mask)
            mask[mask > 0] = 255
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            if not cam_name.endswith('.png') or cam_name.endswith('.jpg'):
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    if os.path.exists(os.path.join(path, "transforms_val.json")):
